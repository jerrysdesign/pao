...
gulp.task('copy', function () {
  return gulp.src(cfg.src + 'vendors'+ '/**/*')
    .pipe(gulp.dest(cfg.build + 'assets/vendors' + '/'));
});
...
gulp.task('styles', function(){
  return gulp.src(cfg.src + 'sass/*.scss')
    ...
    .pipe(gulp.dest(cfg.build + 'assets/' + 'css'));
});
...
gulp.task('js', function () {
  ...
  .pipe(plumber())
  .pipe(gulp.dest(cfg.build + 'assets/' + 'scripts'));
});

gulp.task('image-min', function(){
  return gulp.src(cfg.src + 'img/*')
    .pipe(imagemin())
    .pipe(gulp.dest(cfg.build + 'assets/' + 'img'));
});

gulp.task('serve', ['copy', 'styles', 'views', 'js'], function() {
  browserSync.init({
    server: {
      baseDir: cfg.build
    }
...
gulp.task('css-min', function(){
  return gulp.src(cfg.build + 'assets/' + 'css/*.css')
    .pipe(cssnano())
    .pipe(gulp.dest(cfg.build + 'assets/' + 'css'));
});

gulp.task('default', ['serve', 'image-min']);
